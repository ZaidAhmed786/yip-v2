...
const Popup = (props: any) => {
  ...
  const text = props.text ? props.text : 'Call to Pay Your'
  const handleClick = () => {
      ...
      {isOpen && (
        <div className={styles.overlay} onClick={handleClick}>
          <div className={styles.popup} onClick={handleClick}>
            <div className={styles.popupHeader} style={{ background: props.color }}><p>{number}</p></div>
            <div className={styles.popupContent}>
              <img src={`${props?.url}`} alt="Logo" className={styles.logo} style={{ width: Width }} style={{ width: Width }} />
              <h2 className={styles.heading}> {text} <br /> Bill Now </h2>
